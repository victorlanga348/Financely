...
export default function Saidas() {
    ...
    async function fetchData() {
        try {
            ...
            const expenses = transRes.data.filter((t: any) => t.type === 'expense');
        ...
        <div className="min-h-screen bg-slate-950 text-slate-50 flex overflow-hidden">
            ...
            <aside className="w-64 border-r border-white/10 flex flex-col glass-card m-4 rounded-[2rem] hidden lg:flex">
                ...
                <nav className="flex-1 px-4 space-y-2">
                    ...
                    <NavItem icon={<ArrowDownCircle size={20} />} label="Saídas" active onClick={() => navigate('/saidas')} />
            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto overflow-x-hidden custom-scrollbar">
                ...
                <section className="px-4 md:px-8 pb-8 space-y-4 md:space-y-6 z-10">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex-1 flex items-center bg-white/5 border border-white/10 rounded-2xl px-4 py-3 focus-within:ring-2 focus-within:ring-rose-500/50 transition-all">
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {filteredTransactions.map((transaction) => (
                                            <tr key={transaction.id} className="group hover:bg-white/[0.02] transition-colors">
                                                ...
                                                <td className="py-5">
                                                    <span className="px-4 py-1.5 bg-rose-500/10 border border-rose-500/20 rounded-full text-xs font-bold text-rose-400">
                                                ...
                                                <td className="py-5 font-black text-right text-rose-400">
                        ...
                        {filteredTransactions.map((transaction) => (
                            <div key={transaction.id} className="glass-card p-5 rounded-3xl flex flex-col gap-3">
                                <div className="flex justify-between items-start">
                                    ...
                                    <p className="font-black text-rose-400">- {formatCurrency(transaction.amount)}</p>
                                </div>
